...
def check_simulation_result(test_case):
    ...
    with open(sim_log_path, 'r') as f:
        for line in f:
            ...
            match2 = re.search(r"accuracy:\s+(\d+)\s+false out of\s+(\d+)\s+total", line)
            if match2:
                false_preds = match2.group(1)
            # Check for the success string.
            if "----TB FINISH:Test Passed----" in line:
                passed = True

    if passed:
        return f"Test case {test_case}: PASSED (Cycle count: {cycle_count}) (false preds {false_preds})"
    else:
