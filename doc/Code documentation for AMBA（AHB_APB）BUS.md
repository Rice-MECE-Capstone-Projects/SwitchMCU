...
Code documentation for AMBA（AHB_APB）BUS

1. Overview of AMBA Bus we have used

The Advanced High-performance Bus (AHB) and Advanced Peripheral Bus (APB) are integral components of ARM's AMBA (Advanced Microcontroller Bus Architecture) protocol. The AHB is utilized for high-speed data transfers, connecting with the core of the Switch MCU in our design, and aims for a 72MHz clock rate. Its performance is contingent upon the results of synthesis and static timing analysis (STA).

The APB, in contrast, is simpler and caters to lower-speed, lower-bandwidth communication with peripherals, targeting a 36MHz rate. Its actual efficiency also depends on the synthesis process. As parts of the AMBA suite, AHB and APB work in tandem to ensure efficient communication between system modules.

Our design employs the AHB and APB from the AMBA protocol, with some simplifications. The AMBA protocol is a product of ARM. For the complete reference manual on AHB, you can find it here:

AHB: https://developer.arm.com/documentation/ihi0033/latest/

APB: https://developer.arm.com/documentation/ihi0024/latest/

2. Overall Design of the AMBA

**Peripheral Section:**

 

The AMBA section of for the peripheral our switch MCU consists of several components:

AHB Master (Integrated within the Core): This connects directly to the core and handles data transfers on the Advanced High-performance Bus (AHB).

AHB to APB Bridge (Comprised of AHB Slave and APB Master): This bridge facilitates communication between the AHB and Advanced Peripheral Bus (APB), featuring an AHB Slave that interfaces with the AHB and an APB Master for the APB.

APB Slave (For Peripheral Connections): Used to connect peripherals like UART and SPI, the APB Slave acts as a communication medium between these devices and the bus system.

In our overall process design, illustrated in the accompanying diagram, we focus on the SPI as our reference peripheral. Within the rtl_component/ahb2apb_swc directory, the apb_swc file links the APB Master to the AHB Slave. The spictrl_sw file includes the APB Slave, which also serves as the SPI controller.

This part of our documentation briefly explores the architectural design of the bus system, highlighting the integration and interaction between the AHB and APB in our broader system framework.

​                               

![image-20231130213058934](C:\Users\gaoji\AppData\Roaming\Typora\typora-user-images\image-20231130213058934.png)

 

Regarding the overall design of AMBA connection to SRAM, the system is structured into distinct components, each serving a crucial role:

AHB Master: This component is integrated within the Core. It is responsible for initiating and managing the high-speed data transfers and communications on the Advanced High-performance Bus (AHB). Its primary function is to act as the central communicator with other components in the system.

AHB Slave: This is housed within the back_up_plan version of the SRAM. The AHB Slave acts as a responder to the requests made by the AHB Master. It facilitates the reading from and writing to the SRAM, managing the data transfer processes to ensure accurate and efficient access to the stored data.
...
**Brief Explanation of AHB Signals, Referenced from ARM's Official Manual**
...
WBUFFREAD, RBUFFREAD, RBUFFWRITE, RBUFFDATA[31:0]:

These signals are not standard AHB signals and used for debug
...
​    **Brief Explanation of APB Signals, Referencing ARM's Official Manual**
...
The design aspects of the AHB can be categorized into several sections, namely: AHB Master, AHB Slave, APB Master, and APB Slave.
...
When the waveform files that generated the file is in .vcd format which can be viewed using GTKWave for waveform for comparison.
